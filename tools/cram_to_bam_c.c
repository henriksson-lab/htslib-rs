/* Reference C implementation for the cram_to_bam benchmark workload.
 *
 * Converts a CRAM file to a BAM file. Mirrors the Rust
 * src/bin/cram_to_bam.rs binary used in the same perf comparison.
 */
#include <stdint.h>
#include <stdio.h>
#include <stdlib.h>
...
int main(int argc, char **argv) {
    if (argc != 3) {
        fprintf(stderr, "usage: cram_to_bam_c <in.cram> <out.bam>\n");
        return 2;
    }
    htsFile *in_fp = hts_open(argv[1], "rc");
