...
unsafe fn hfile_plugin_destroy_fn(ptr: *const ()) -> HFilePluginDestroyFn {
...
impl HFileBackend {
    ...
    // `*mut ()`); genuine OS syscalls (read/write/lseek/close/fdatasync)
            ...
            HFileBackend::Fd { fd, flags } => {
                ...
                loop {
                    let n = if (flags & HFILE_FD_IS_SOCKET) != 0 {
                        libc::recv(fd, ptr.cast(), nbytes, 0)
                    } else {
                        libc::read(fd, ptr.cast(), nbytes)
                    };
    ...
    pub unsafe fn write(fp: &mut hFILE, src: &[u8]) -> libc::ssize_t {
        ...
        match &fp.backend {
            HFileBackend::Fd { fd, flags } => {
                ...
                loop {
                    let n = if (flags & HFILE_FD_IS_SOCKET) != 0 {
                        libc::send(fd, ptr.cast(), nbytes, 0)
                    } else {
                        libc::write(fd, ptr.cast(), nbytes)
                    };
                    if !(n < 0 && *libc::__errno_location() == libc::EINTR) {
                        return n;
                    }
...
pub unsafe fn hfile_init(struct_size: usize, mode: *const u8, capacity: usize) -> *mut hFILE {
...
pub unsafe fn hfile_c_104_hfile_init(
    ...
    let Some(mode) = (!mode.is_null()).then(|| CStr::from_ptr(mode.cast()).to_bytes()) else {
        *libc::__errno_location() = libc::EINVAL;
...
pub unsafe fn htslib_hfile_h_275_hputs(text: *const u8, fp: *mut hFILE) -> i32 {
    let Some(fp) = fp.as_mut() else {
        return libc::EOF;
    };
    let Some(text) = (!text.is_null()).then(|| CStr::from_ptr(text.cast()).to_bytes()) else {
        return libc::EOF;
    };
    hputs(text, fp)
}
...
pub unsafe fn hfile_c_664_hopen_fd(filename: *const u8, mode: *const u8) -> *mut hFILE {
    let Some(mode_bytes) = (!mode.is_null()).then(|| CStr::from_ptr(mode.cast()).to_bytes()) else {
        *libc::__errno_location() = libc::EINVAL;
        return std::ptr::null_mut();
    };
    let fd = libc::open(filename.cast(), hfile_oflags_bytes(mode_bytes), 0o666);
    if fd < 0 {
        return std::ptr::null_mut();
    }
...
pub unsafe fn hfile_c_726_is_preload_url_remote(url: *const u8) -> i32 {
    hisremote(url.add(8))
}

pub unsafe fn hfile_c_730_hopen_preload(url: *const u8, mode: *const u8) -> *mut hFILE {
...
pub unsafe fn hfile_c_735_hdopen(fd: i32, mode: *const u8) -> *mut hFILE {
    let Some(mode_bytes) = (!mode.is_null()).then(|| CStr::from_ptr(mode.cast()).to_bytes()) else {
        *libc::__errno_location() = libc::EINVAL;
        return std::ptr::null_mut();
    };
...
pub unsafe fn hfile_c_747_hopen_fd_fileuri(
    ...
    if libc::strncmp(url.cast(), c"file://localhost/".as_ptr(), 17) == 0 {
        url = url.add(16);
    } else if libc::strncmp(url.cast(), c"file:///".as_ptr(), 8) == 0 {
        url = url.add(7);
    } else {
...
pub unsafe fn hfile_c_761_hopen_fd_stdinout(mode: *const u8) -> *mut hFILE {
    let Some(mode_bytes) = (!mode.is_null()).then(|| CStr::from_ptr(mode.cast()).to_bytes()) else {
        *libc::__errno_location() = libc::EINVAL;
        return std::ptr::null_mut();
    };
...
pub unsafe fn hfile_c_772_hfile_oflags(mode: *const u8) -> i32 {
    let Some(mode) = (!mode.is_null()).then(|| CStr::from_ptr(mode.cast()).to_bytes()) else {
        return 0;
    };
...
pub unsafe fn hfile_c_826_cmp_prefix(mut key: *const u8, mut s: *const u8) -> i32 {
    while *key != 0 {
        if tolower_c(*s) != *key {
            return 1;
        }
...
pub unsafe extern "C" fn hfile_c_845_hopen_mem(
    ...
    let Some(url) = (!url.is_null()).then(|| CStr::from_ptr(url.cast()).to_bytes()) else {
        *libc::__errno_location() = libc::EINVAL;
        return std::ptr::null_mut();
    };
    let Some(mode_bytes) = (!mode.is_null()).then(|| CStr::from_ptr(mode.cast()).to_bytes()) else {
...
pub unsafe fn hfile_c_878_hopenv_mem(
    ...
    // RECONVERGE: `buffer` is a C-ABI-supplied block whose ownership transfers to
    // this function; it must be released with C `free()` (not a Rust drop).
...
pub unsafe fn hfile_mem_get_buffer_impl(
    ...
    file.buffer.as_mut_ptr().cast::<u8>()
}
...
pub unsafe fn hfile_mem_steal_buffer_impl(
    ...
    // RECONVERGE: genuine C-ABI ownership transfer — the caller frees this block
    // with C `free()`, so it must remain a C malloc (cannot be an owned Vec/Box).
    let stolen = libc::malloc(capacity as usize).cast::<u8>();
    if stolen.is_null() {
        *libc::__errno_location() = libc::ENOMEM;
        return std::ptr::null_mut();
    }
...
pub unsafe fn hfile_c_291_hgets(buffer: *mut u8, size: i32, fp: *mut hFILE) -> *mut u8 {
    let Some(fp) = fp.as_mut() else {
        return std::ptr::null_mut();
    };
    if size < 1 {
...
pub unsafe extern "C" fn hfile_c_935_crypt4gh_needed(
    ...
    let _u = if libc::strncmp(url.cast(), c"crypt4gh:".as_ptr(), 9) == 0 {
        url.add(9)
    } else {
        url
    };
...
unsafe extern "C" fn hfile_c_1116_preload_isremote(fname: *const u8) -> i32 {
...
unsafe extern "C" fn hfile_c_1178_unknown_isremote(fname: *const u8) -> i32 {
...
pub unsafe fn hfile_c_1176_find_scheme_handler(s: *const u8) -> *const hFILE_scheme_handler {
    ...
    while i < scheme.len() {
        let c = *s.add(i);
        if isalnum_c(c) != 0 || c == b'+' as u8 || c == b'-' as u8 || c == b'.' as u8 {
...
pub unsafe extern "C" fn hfile_c_1342_hfile_always_remote(_fname: *const u8) -> i32 {
    1
}
...
pub unsafe fn hfile_c_1257_hfile_list_plugins(
    ...
    let mut np = 0;
    if np < *nplugins {
        *plist.add(np as usize) = c"built-in".as_ptr().cast::<u8>();
    }
    np += 1;
...
pub unsafe fn hfile_c_1293_hfile_has_plugin(name: *const u8) -> i32 {
    {
        let needs_load = hfile_plugin_state().lock().unwrap().schemes.is_none();
        if needs_load && hfile_c_1111_load_hfile_plugins() < 0 {
            return -1;
    ...
    let state = hfile_plugin_state().lock().unwrap();
    for p in state.plugins.iter().rev() {
        if libc::strcmp(p.plugin.name.cast(), name.cast()) == 0 {
            return 1;
        }
    }
...
pub unsafe fn hfile_c_1364_haddextension(
    ...
    let trailing = if !hfile_c_1176_find_scheme_handler(filename).is_null() {
        ...
        {
            libc::strcspn(filename.cast(), c"?#".as_ptr())
        } else {
            libc::strcspn(filename.cast(), c"?".as_ptr())
        };
        filename.add(span)
    } else {
        filename.add(CStr::from_ptr(filename.cast()).to_bytes().len())
    };
    ...
    let new_extension_slice = CStr::from_ptr(new_extension.cast()).to_bytes();
    let trailing_slice = CStr::from_ptr(trailing.cast()).to_bytes();
...
pub unsafe fn hfile_c_1416_knet_open(fn_: *const u8, mode: *const u8) -> *mut knetFile {
    let Some(filename) = (!fn_.is_null()).then(|| CStr::from_ptr(fn_.cast()).to_bytes()) else {
        *libc::__errno_location() = libc::EINVAL;
        return std::ptr::null_mut();
    };
...
pub unsafe fn hfile_c_1433_knet_dopen(fd: i32, mode: *const u8) -> *mut knetFile {
...
pub unsafe fn hfile_oflags(mode: *const u8) -> i32 {
    hfile_c_772_hfile_oflags(mode)
}

pub unsafe fn hdopen(fd: i32, mode: *const u8) -> *mut hFILE {
    hfile_c_735_hdopen(fd, mode)
}

pub unsafe fn hisremote(filename: *const u8) -> i32 {
...
pub unsafe fn hgets(buffer: *mut u8, size: i32, fp: *mut hFILE) -> *mut u8 {
    let Some(fp) = hfile_mut(fp) else {
        return std::ptr::null_mut();
    };
    if size < 1 {
...
pub unsafe fn hpeek(fp: *mut hFILE, buffer: *mut (), nbytes: usize) -> libc::ssize_t {
    let Some(fp) = hfile_mut(fp) else {
        *libc::__errno_location() = libc::EINVAL;
        return -1;
    };
...
pub unsafe fn hfile_mem_get_buffer(file: *mut hFILE, length: *mut usize) -> *mut u8 {
...
pub unsafe fn hfile_mem_steal_buffer(file: *mut hFILE, length: *mut usize) -> *mut u8 {
    let Some(file) = hfile_mut(file) else {
        *libc::__errno_location() = libc::EINVAL;
        return std::ptr::null_mut();
    };
...
pub unsafe fn hfile_list_plugins(plist: *mut *const u8, nplugins: *mut i32) -> i32 {
    hfile_c_1257_hfile_list_plugins(plist, nplugins)
}

pub unsafe fn hfile_has_plugin(name: *const u8) -> i32 {
...
pub unsafe fn hopen(fname: *const u8, mode: *const u8) -> *mut hFILE {
    hfile_c_1317_hopen(fname, mode)
}

pub unsafe fn knet_open(fn_: *const u8, mode: *const u8) -> *mut knetFile {
    hfile_c_1416_knet_open(fn_, mode)
}

pub unsafe fn knet_dopen(fd: i32, mode: *const u8) -> *mut knetFile {
    hfile_c_1433_knet_dopen(fd, mode)
}

pub unsafe fn knet_read(fp: *mut knetFile, buf: *mut (), len: usize) -> libc::ssize_t {
...
mod tests {
    ...
    fn hfile_init_builds_owned_mobile_read_buffer() {
        unsafe {
            let fp = hfile_c_104_hfile_init(0, c"r".as_ptr().cast::<u8>(), 999_999);
            assert!(!fp.is_null());
    ...
    fn hfile_destroy_and_abrupt_close_preserve_errno() {
        unsafe {
            let fp = hfile_c_104_hfile_init(0, c"r".as_ptr().cast::<u8>(), 16);
            assert!(!fp.is_null());
            *libc::__errno_location() = libc::E2BIG;
            ...
            let abrupt = hfile_c_104_hfile_init(0, c"r".as_ptr().cast::<u8>(), 16);
    ...
    fn hfile_init_reports_allocation_failure_without_leaking() {
        unsafe {
            *libc::__errno_location() = 0;
            let fp = hfile_c_104_hfile_init(0, c"w".as_ptr().cast::<u8>(), usize::MAX);
            assert!(fp.is_null());
    ...
    fn owned_and_borrowed_hfile_wrap_a_mem_backend() {
        unsafe {
            ...
            let fp = hfile_c_845_hopen_mem(c"data:,abc".as_ptr().cast::<u8>(), c"r".as_ptr().cast::<u8>());
            assert!(!fp.is_null());
    ...
    fn owned_hfile_closes_fd_backend_on_drop() {
        unsafe {
            ...
            let fp = hfile_c_664_hopen_fd(c_path.as_ptr().cast(), c"r".as_ptr().cast::<u8>());
            assert!(!fp.is_null());
    ...
    fn hfile_memory_backend_decodes_data_urls_and_exposes_buffer() {
        unsafe {
            let fp = hfile_c_845_hopen_mem(c"data:,hello%2C%20world%21".as_ptr().cast::<u8>(), c"r".as_ptr().cast::<u8>());
            assert!(!fp.is_null());
            ...
            let fp64 = hfile_c_845_hopen_mem(c"data:;base64,QUJDRA==".as_ptr().cast::<u8>(), c"r".as_ptr().cast::<u8>());
            assert!(!fp64.is_null());
            ...
            assert!(hfile_c_845_hopen_mem(c"data:,x".as_ptr().cast::<u8>(), c"w".as_ptr().cast::<u8>()).is_null());
            assert_eq!(*libc::__errno_location(), libc::EROFS);
            assert!(hfile_c_915_hopen_not_supported(c"mem".as_ptr().cast::<u8>(), c"r".as_ptr().cast::<u8>()).is_null());
            assert_eq!(*libc::__errno_location(), libc::EINVAL);
        }
    }
    ...
    fn hfile_mem_steal_buffer_rejects_non_mem_backend() {
        unsafe {
            let payload = b"owned";
            let buffer = libc::malloc(8).cast::<u8>();
            ...
            let fp = hfile_c_835_create_hfile_mem(buffer, c"r".as_ptr().cast::<u8>(), payload.len(), 8);
            assert!(!fp.is_null());
            ...
            let nonmem = hfile_c_104_hfile_init(0, c"r".as_ptr().cast::<u8>(), 8);
    ...
    fn hfile_data_url_decoder_keeps_edge_case_semantics() {
        unsafe {
            assert!(hfile_c_845_hopen_mem(c"data:no-comma".as_ptr().cast::<u8>(), c"r".as_ptr().cast::<u8>()).is_null());
            assert_eq!(*libc::__errno_location(), libc::EINVAL);

            let fp = hfile_c_845_hopen_mem(c"data:text/plain,kept%ZZ%2f".as_ptr().cast::<u8>(), c"r".as_ptr().cast::<u8>());
            assert!(!fp.is_null());
            let mut out = [0u8; 16];
            ...
            let empty = hfile_c_845_hopen_mem(c"data:,".as_ptr().cast::<u8>(), c"r".as_ptr().cast::<u8>());
    ...
    fn hfile_mem_getc_getdelim_hgets_and_khgetline() {
        unsafe {
            ...
            assert_eq!(std::ffi::CStr::from_ptr(line.as_ptr().cast()).to_bytes(), b"b\n");

            assert_eq!(
                hfile_c_291_hgets(line.as_mut_ptr(), line.len() as i32, fp),
                line.as_mut_ptr()
            );
            assert_eq!(std::ffi::CStr::from_ptr(line.as_ptr().cast()).to_bytes(), b"cdef\n");
    ...
    fn hfile_fd_backend_opens_reads_and_honours_shared_and_uri() {
        unsafe {
            ...
            assert!(fd >= 0);
            let shared = hfile_c_735_hdopen(fd, c"Sr".as_ptr().cast::<u8>());
            ...
            let fp_uri = hfile_c_747_hopen_fd_fileuri(file_uri.as_ptr().cast(), c"r".as_ptr().cast::<u8>());
            assert!(!fp_uri.is_null());
            assert_eq!(hclose(fp_uri), 0);

            assert!(hfile_c_747_hopen_fd_fileuri(
                c"http://example.invalid/x".as_ptr().cast::<u8>(),
                c"r".as_ptr().cast::<u8>()
            )
            .is_null());
            assert_eq!(
    ...
    fn hfile_fd_write_flush_and_read_roundtrip_via_socketpair() {
        unsafe {
            ...
            let reader = hfile_c_735_hdopen(fds[0], c"sr".as_ptr().cast::<u8>());
            let writer = hfile_c_735_hdopen(fds[1], c"sw".as_ptr().cast::<u8>());
    ...
    fn hfile_fd_putc_hputs_hwrite_buffer_then_persist_to_file() {
        unsafe {
            ...
            let fp = hfile_c_664_hopen_fd(c_path.as_ptr().cast(), c"w".as_ptr().cast::<u8>());
            assert!(!fp.is_null());
            assert_eq!(htslib_hfile_h_263_hputc(b'a' as i32, fp), b'a' as i32);
            assert_eq!(htslib_hfile_h_275_hputs(c"bc".as_ptr().cast::<u8>(), fp), 0);
    ...
    fn hfile_preload_copies_fd_stream_into_immobile_memory_file() {
        unsafe {
            ...
            assert_eq!(hfile_c_726_is_preload_url_remote(preload_url.as_ptr().cast()), 0);
    ...
    fn hfile_extension_helpers_match_local_and_url_paths() {
        unsafe {
            assert_eq!(hfile_c_1339_hfile_always_local(c"x".as_ptr().cast::<u8>()), 0);
            assert_eq!(hfile_c_1342_hfile_always_remote(c"x".as_ptr().cast::<u8>()), 1);
            assert_eq!(hisremote(c"/tmp/a.bam".as_ptr().cast::<u8>()), 0);
            assert_eq!(hisremote(c"data:,abc".as_ptr().cast::<u8>()), 0);
            assert_eq!(hisremote(c"file:///tmp/a.bam".as_ptr().cast::<u8>()), 0);
            assert_eq!(hisremote(c"mem:payload".as_ptr().cast::<u8>()), 1);
            assert_eq!(hisremote(c"preload:mem:payload".as_ptr().cast::<u8>()), 1);
            ...
            let ext = hfile_c_1353_strip_extension(path.as_ptr().cast::<u8>(), limit);
            assert_eq!(CStr::from_ptr(ext.cast()).to_bytes(), b".bam");

            let mut ks = kstring_t { data: Vec::new() };
            let out =
                hfile_c_1364_haddextension(&mut ks, c"/tmp/a.bam".as_ptr().cast::<u8>(), 1, c".csi".as_ptr().cast::<u8>());
            assert!(!out.is_null());
            assert_eq!(ks.data.as_slice(), b"/tmp/a.csi");

            let out2 = hfile_c_1364_haddextension(
                &mut ks,
                c"file:///tmp/a.bam?x=1".as_ptr().cast::<u8>(),
                1,
                c".csi".as_ptr().cast::<u8>(),
            );
            assert!(!out2.is_null());
    ...
    fn hfile_oflags_matches_fopen_mode_letters() {
        unsafe {
            assert_eq!(
                hfile_c_772_hfile_oflags(c"r".as_ptr().cast::<u8>()) & libc::O_ACCMODE,
                libc::O_RDONLY
            );
            assert_eq!(
                hfile_c_772_hfile_oflags(c"w".as_ptr().cast::<u8>()),
                libc::O_WRONLY | libc::O_CREAT | libc::O_TRUNC
            );
            assert_eq!(
                hfile_c_772_hfile_oflags(c"a+".as_ptr().cast::<u8>())
                    & (libc::O_ACCMODE | libc::O_CREAT | libc::O_APPEND),
                libc::O_RDWR | libc::O_CREAT | libc::O_APPEND
            );
            assert_ne!(hfile_c_772_hfile_oflags(c"wx".as_ptr().cast::<u8>()) & libc::O_EXCL, 0);
        }
    }
    ...
    fn hfile_hopen_dispatches_builtin_and_unknown_schemes() {
        unsafe {
            let fp = hfile_c_1317_hopen(c"data:,builtin%20dispatch".as_ptr().cast::<u8>(), c"r".as_ptr().cast::<u8>());
            assert!(!fp.is_null());
            let mut out = [0u8; 32];
            ...
            let missing =
                hfile_c_1168_hopen_unknown_scheme(c"missing-scheme:test".as_ptr().cast::<u8>(), c"r".as_ptr().cast::<u8>());
    ...
    fn hfile_mem_vopen_decodes_va_list_buffer_and_size() {
        unsafe {
            ...
            let buffer = libc::malloc(payload.len()).cast::<u8>();
            assert!(!buffer.is_null());
            libc::memcpy(
                buffer.cast(),
                payload.as_ptr().cast(),
    ...
    fn hfile_plugin_initialisers_set_names_and_error_handler_state() {
        ...
        unsafe {
            ...
            assert_eq!(CStr::from_ptr(mem_plugin.name.cast()).to_bytes(), b"mem");
            ...
            assert_eq!(CStr::from_ptr(crypt_plugin.name.cast()).to_bytes(), b"crypt4gh-needed");

            assert!(
                hfile_c_935_crypt4gh_needed(c"crypt4gh:/tmp/x".as_ptr().cast::<u8>(), c"r".as_ptr().cast::<u8>()).is_null()
            );
            assert_eq!(
                *libc::__errno_location(),
                libc::EPROTONOSUPPORT
    ...
    fn hfile_knet_wrappers_track_offset_and_delegate_to_hfile() {
        unsafe {
            ...
            let fp = hfile_c_1416_knet_open(c_path.as_ptr().cast(), c"r".as_ptr().cast::<u8>());
            assert!(!fp.is_null());
