...
pub unsafe fn test_thrash_threads5_c_35_main(argc: c_int, argv: *mut *mut c_char) -> c_int {
    ...
    let p = if argc > 2 {
        let p = crate::htslib_rs::thread_pool::hts_tpool_init(libc::atoi(*argv.add(2)));
        crate::htslib_rs::bgzf::bgzf_thread_pool(fpin, p, 0);
        p
    } else {
        ptr::null_mut()
    };
    ...
    libc::fprintf(
        hts_sys::stderr.cast(),
        c"close=%d\n".as_ptr(),
        crate::htslib_rs::bgzf::bgzf_close(fpin) as c_int,
    );
    if !p.is_null() {
