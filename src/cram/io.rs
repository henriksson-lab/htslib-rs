...
use std::ffi::{c_char, c_int, c_uchar, c_uint, c_ulong, c_void, CStr, CString};
use libc::size_t;
...
pub unsafe fn cram_cram_io_c_145_cram_decode_compression_header(
    fd: *mut cram_fd,
    b: *mut cram_block,
...
pub unsafe fn cram_cram_io_c_955_cram_decode_slice_header(
    fd: *mut cram_fd,
    b: *mut cram_block,
...
pub unsafe fn cram_cram_io_c_4330_cram_new_compression_header(
    ...
    (*hdr).td_blk = cram_cram_io_c_1388_cram_new_block(crate::htslib_rs::cram::CRAM_CONTENT_TYPE_CORE, 0).cast();
...
pub unsafe fn cram_cram_io_c_4889_cram_write_SAM_hdr(
    ...
    if !(*fdl).refs.is_null() && (*fdl).no_ref == 0 && (*fdl).embed_ref <= 1 {
        ...
        while i < nref {
            ...
            if crate::htslib_rs::sam::sam_hrecs_find_key(
                ty,
                c"M5".as_ptr(),
                std::ptr::null_mut(),
            )
            ...
            if !(*fdl).ref_fn.is_null() {
                let mut ref_fn_buf = [0 as c_char; libc::PATH_MAX as usize];
    ...
    if major == 1 {
        ...
        if major >= 3
            && cram_cram_io_c_2323_cram_compress_block(
                fd,
                b,
                std::ptr::null_mut(),
                -1,
        ...
        if blank_block {
            ...
            *(*cl).landmark.add(1) = (*cl).length;
            ...
            (*cl).length += padded_length
                ...
                + crc_extra
                + varint_size((*bl).content_id as i64) as i32
                + varint_size(padded_length as i64) as i32 * 2;
        } else {
...
pub unsafe fn cram_cram_io_c_5558_cram_close(fd: *mut cram_fd) -> c_int {
    ...
    if ret == 0 && (*fdl).mode == b'w' as c_int {
        if 0 != cram_cram_io_c_5474_cram_write_eof_block(fd) {
            ret = -1;
        }
    }
...
pub unsafe fn cram_cram_io_c_3639_cram_new_container(
    ...
    (*c).tags_used = calloc(1, std::mem::size_of::<kh_generic_layout>() as u64)
        .cast::<kh_generic_layout>();
...
pub unsafe fn cram_cram_io_c_5474_cram_write_eof_block(fd: *mut cram_fd) -> c_int {
    ...
    if major >= 2 {
        ...
        if cram_write_container(fd, (&mut c as *mut cram_container_layout).cast::<cram_container>()) < 0
...
pub unsafe fn cram_cram_io_c_3788_cram_read_container(
    ...
    (*fdl).empty_container = if (*c).num_records == 0
        && (*c).ref_seq_id == -1
        && (*c).ref_seq_start == 0x454f46
    {
        1
    } else {
        0
...
pub unsafe fn cram_cram_io_c_4506_cram_new_slice(
    ...
    (*s).pair[0] = calloc(1, std::mem::size_of::<kh_generic_layout>() as u64)
        .cast::<kh_generic_layout>();
    (*s).pair[1] = calloc(1, std::mem::size_of::<kh_generic_layout>() as u64)
        .cast::<kh_generic_layout>();
...
pub unsafe fn cram_cram_io_c_4568_cram_read_slice(
    ...
    match (*bl).content_type {
        x if x == CRAM_CONTENT_TYPE_MAPPED_SLICE || x == CRAM_CONTENT_TYPE_UNMAPPED_SLICE => {
            (*s).hdr = cram_cram_io_c_955_cram_decode_slice_header(fd.cast(), b.cast()).cast();
            if (*s).hdr.is_null() {
                fail!();
            }
        }
    ...
    let n = (*(*s).hdr).num_blocks;
    (*s).block = calloc(n as u64, std::mem::size_of::<*mut cram_block_layout>() as u64)
