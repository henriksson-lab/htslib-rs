...
pub unsafe fn bgzip_c_217_main(argc: c_int, argv: *mut *mut c_char) -> c_int {
    ...
    let mut fp: *mut crate::htslib_mini_rs::hts::BGZF = ptr::null_mut();
    ...
    loop {
        ...
        match c {
            ...
            x if x == b'b' as c_int => {
                start = libc::atol(libc::optarg);
                compress = 0;
                pstdout = 1;
            }
            x if x == b's' as c_int => {
                size = libc::atol(libc::optarg);
                pstdout = 1;
            }
            x if x == b'f' as c_int => is_forced += 1,
            x if x == b'i' as c_int => index = 1,
            x if x == b'I' as c_int => index_fname = libc::optarg,
            x if x == b'l' as c_int => compress_level = libc::atol(libc::optarg) as c_int,
            x if x == b'g' as c_int => rebgzip = 1,
            x if x == b'r' as c_int => {
                reindex = 1;
                compress = 0;
            }
            x if x == b'@' as c_int => threads = libc::atoi(libc::optarg),
            x if x == b't' as c_int => {
                test = 1;
                compress = 0;
                reindex = 0;
            ...
            x if x == b'o' as c_int => write_fname = libc::optarg,
            1 => {
                libc::printf(
    ...
    loop {
        isstdin = if libc::optind >= argc {
            1
        } else {
            (libc::strcmp(c"-".as_ptr(), *argv.add(libc::optind as usize)) == 0) as c_int
        };
        ...
        if compress == 1 {
            let mut f_src: *mut crate::htslib_mini_rs::hts::hFILE = ptr::null_mut();
            ...
            } else if argc > libc::optind && isstdin == 0 {
                if pstdout != 0 {
                    fp = bgzf_open(c"-".as_ptr(), out_mode.as_ptr());
                } else {
                    let name = libc::malloc(libc::strlen(*argv.add(libc::optind as usize)) + 5)
                        .cast::<c_char>();
                    libc::strcpy(name, *argv.add(libc::optind as usize));
            ...
            if index != 0 && write_fname.is_null() {
                if !index_fname.is_null() {
                    ...
                    if bgzf_index_dump(fp, *argv.add(libc::optind as usize), c".gz.gzi".as_ptr())
                        < 0
                    {
                        libc::fprintf(
                            hts_sys::stderr.cast(),
            ...
            if !statfilename.is_null() {
                if bgzip_c_114_getfilespec(*argv.add(libc::optind as usize), &mut filestat) == 0 {
            ...
            if argc > libc::optind
                ...
                && write_fname.is_null()
            {
                libc::unlink(*argv.add(libc::optind as usize));
            }

            libc::free(buffer.cast());
        } else if reindex != 0 {
            if argc > libc::optind && isstdin == 0 {
                fp = bgzf_open(*argv.add(libc::optind as usize), c"r".as_ptr());
                if fp.is_null() {
                    libc::fprintf(
                        hts_sys::stderr.cast(),
            ...
            if !index_fname.is_null() {
                ...
                if bgzf_index_dump(fp, *argv.add(libc::optind as usize), c".gzi".as_ptr()) < 0 {
                    libc::fprintf(
                        hts_sys::stderr.cast(),
            ...
            if argc > libc::optind && isstdin == 0 {
                ...
                if pstdout != 0 || test != 0 {
                    ...
                    let name = libc::strdup(*argv.add(libc::optind as usize));
                    if name.is_null() {
                        libc::fprintf(
                            hts_sys::stderr.cast(),
            ...
            if start > 0 {
                if !index_fname.is_null() {
                    ...
                    if bgzf_index_load(fp, *argv.add(libc::optind as usize), c".gzi".as_ptr()) < 0 {
                        libc::fprintf(
                            hts_sys::stderr.cast(),
