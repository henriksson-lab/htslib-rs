use crate::htslib_mini_rs::{cram as cram_api, sam};
...
unsafe fn cram_copy_slice(
    ...
    for _ in 0..num_slice {
        let mut blk = cram_api::cram_read_block(in_);
        if blk.is_null() {
            return -1;
        }

        let hdr = cram_api::cram_decode_slice_header(in_, blk);
        if hdr.is_null() {
            cram_api::cram_cram_io_c_1565_cram_free_block(blk);
            return -1;
        }

        if cram_api::cram_write_block(out, blk) != 0 {
            cram_api::cram_cram_io_c_1565_cram_free_block(blk);
            return -1;
        }
        cram_api::cram_cram_io_c_1565_cram_free_block(blk);
        ...
        for _ in 0..num_blocks {
            blk = cram_api::cram_read_block(in_);
            if blk.is_null() || cram_api::cram_write_block(out, blk) != 0 {
                if !blk.is_null() {
                    cram_api::cram_cram_io_c_1565_cram_free_block(blk);
                }
                return -1;
...
pub unsafe fn cram_cram_external_c_776_cram_filter_container(
    ...
    let blk = cram_api::cram_read_block(in_);
    if blk.is_null() {
        return -1;
    }
    (*c_layout).comp_hdr = cram_api::cram_decode_compression_header(in_, blk).cast();
    (*in_fd).ctr = c_layout;
    ...
    if (*c_layout).ref_seq_id == -2 {
            ...
            && (*in_fd).range.end >= (i64::MAX & ((0xffff_ffff_u64 << 32) as i64))
        {
            ...
            err |= (cram_api::cram_write_container(out, c) < 0) as c_int;
            err |= cram_api::cram_write_block(out, blk);
...
pub unsafe fn cram_cram_external_c_934_cram_transcode_rg(
    ...
    let o_blk = cram_api::cram_read_block(in_);
    let old_size = cram_api::cram_block_size(o_blk) as c_int;
    let ch = cram_api::cram_decode_compression_header(in_, o_blk);
    ...
    if cram_api::cram_write_container(out, c) != 0 {
        return -2;
    }

    cram_api::cram_write_block(out, n_blk);
