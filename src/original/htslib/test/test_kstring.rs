...
pub unsafe fn test_test_kstring_c_458_test_kinsertchar() -> c_int {
    ...
    for i in 0..7 {
        ...
        if crate::htslib_mini_rs::hts::kinsert_char((b'A' as c_int + i) as c_char, t.l, &mut t) < 0
        {
            libc::fprintf(
                hts_sys::stderr.cast(),
...
pub unsafe fn test_test_kstring_c_514_test_kinsertstr() -> c_int {
    ...
    let old_len = res.l;
    if crate::htslib_mini_rs::hts::kinsert_str(c"".as_ptr(), 1, &mut res) != 0 || old_len != res.l {
        libc::fprintf(
            hts_sys::stderr.cast(),
