...
use crate::htslib_mini_rs::hts::BGZF;
...
pub unsafe fn test_thrash_threads6_c_34_main(argc: c_int, argv: *mut *mut c_char) -> c_int {
    ...
    let mut uend: u64 = 0;
    for _ in 0..100 {
        let got = crate::htslib_mini_rs::bgzf::bgzf_read(fpin, buf.as_mut_ptr().cast(), 65536);
        if got < 0 {
            libc::abort();
        }
        upos += got as u64;
    ...
    crate::htslib_mini_rs::bgzf::bgzf_close(fpin);
    ...
    for i in 0..1000 {
        ...
        for _ in 0..80 {
            ...
            libc::fflush(hts_sys::stdout);
            match n {
                0 => {
                    if crate::htslib_mini_rs::bgzf::bgzf_seek(fpin, 0, libc::SEEK_SET) < 0 {
                        libc::puts(c"!".as_ptr());
                    }
                    eof = 0;
                }
                1 => {
                    if crate::htslib_mini_rs::bgzf::bgzf_seek(fpin, pos, libc::SEEK_SET) < 0 {
                        libc::puts(c"!".as_ptr());
                    }
                    eof = 0;
                }
                2 => {
                    if crate::htslib_mini_rs::bgzf::bgzf_seek(fpin, end, libc::SEEK_SET) < 0 {
                        libc::puts(c"!".as_ptr());
                    }
                    eof = 1;
                }
                3 | 4 => {
                    ...
                    if crate::htslib_mini_rs::bgzf::bgzf_read(
                        fpin,
                ...
                5 => {
                    crate::htslib_mini_rs::hts::hts_usleep(N);
                }
                6 => {
                    if mt == 0 {
                        crate::htslib_mini_rs::bgzf::bgzf_mt(fpin, 8, 256);
                    }
                    mt = 1;
                }
                _ => {}
        ...
        libc::printf(c"\n".as_ptr());
        if crate::htslib_mini_rs::bgzf::bgzf_close(fpin) != 0 {
            libc::abort();
        }
    }
