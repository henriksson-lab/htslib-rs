use crate::htslib_mini_rs::{
    hts::{
        self, htsFile, htsThreadPool, hts_idx_t, hts_itr_t, HTS_FORMAT_SEQUENCE_DATA,
...
pub unsafe fn test_test_view_c_65_sam_loop(
    ...
    if optind + 1 < argc && ((*opts).flag & READ_COMPRESSED) == 0 {
        // BAM input and has a region
        if {
            idx = sam::sam_index_load(in_, *argv.add(optind as usize));
            idx.is_null()
        } {
            libc::fprintf(
                hts_sys::stderr.cast(),
        ...
        if (*opts).multi_reg != 0 {
            ...
            while {
                ...
                if (*opts).benchmark == 0 && sam::sam_c_4553_sam_write1(out, h, b) < 0 {
                    libc::fprintf(hts_sys::stderr.cast(), c"Error writing output.\n".as_ptr());
            ...
            hts::hts_itr_destroy(iter);
            if r < -1 {
                libc::fprintf(hts_sys::stderr.cast(), c"Error reading input.\n".as_ptr());
                if !b.is_null() {
                    sam::bam_destroy1(b);
                }
                if !h.is_null() {
    ...
    if r < -1 {
        libc::fprintf(hts_sys::stderr.cast(), c"Error parsing input.\n".as_ptr());
        if !b.is_null() {
            sam::bam_destroy1(b);
        }
        if !h.is_null() {
    ...
    if !(*opts).index.is_null() {
        if sam::sam_idx_save(out) < 0 {
            libc::fprintf(hts_sys::stderr.cast(), c"Error saving index\n".as_ptr());
            if !b.is_null() {
                sam::bam_destroy1(b);
            }
            if !h.is_null() {
...
pub unsafe fn test_test_view_c_196_vcf_loop(
    ...
    if optind + 1 < argc {
        // A series of regions.
        idx = vcf::bcf_index_load2(*argv.add(optind as usize), std::ptr::null());
        if idx.is_null() {
            libc::fprintf(
                hts_sys::stderr.cast(),
        ...
        let mut i = optind + 1;
        while i < argc {
            let iter = vcf::vcf_c_4831_bcf_itr_querys1(idx, h, *argv.add(i as usize));
            if iter.is_null() {
                libc::fprintf(
                    hts_sys::stderr.cast(),
                    c"[E::%s] fail to parse region '%s'\n".as_ptr(),
            ...
            while {
                r = vcf::htslib_vcf_h_1327_bcf_itr_next(in_, iter, b.cast());
                r >= 0
            } {
...
pub unsafe fn test_test_view_c_279_main(argc: c_int, argv: *mut *mut c_char) -> c_int {
    ...
    if argc == optind {
        ...
        libc::fprintf(hts_sys::stderr.cast(), c"-v: increase verbosity\n".as_ptr());
        libc::fprintf(
            hts_sys::stderr.cast(),
    ...
    in_ = hts::hts_open(*argv.add(optind as usize), moder.as_ptr());
    if in_.is_null() {
        libc::fprintf(
            hts_sys::stderr.cast(),
            c"Error opening \"%s\"\n".as_ptr(),
    ...
    match (*hts::hts_get_format(in_)).category {
        HTS_FORMAT_SEQUENCE_DATA => {
            ret = test_test_view_c_65_sam_loop(argc, argv, optind, &mut opts, in_, out);
        }

        HTS_FORMAT_VARIANT_DATA => {
            ret = test_test_view_c_196_vcf_loop(argc, argv, optind, &mut opts, in_, out);
        }
    ...
    if ret < 0 {
        libc::fprintf(hts_sys::stderr.cast(), c"Error closing output.\n".as_ptr());
        exit_code = libc::EXIT_FAILURE;
    }
    ret = hts::hts_close(in_);
    if ret < 0 {
        libc::fprintf(hts_sys::stderr.cast(), c"Error closing input.\n".as_ptr());
        exit_code = libc::EXIT_FAILURE;
    }
    ...
    if libc::fclose(hts_sys::stdout) != 0
