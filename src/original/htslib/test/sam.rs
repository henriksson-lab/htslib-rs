...
pub unsafe fn test_sam_c_705_use_header_api() {
    ...
    let in_ = crate::htslib_mini_rs::hts::hts_open(header_text.as_ptr(), c"r".as_ptr());
    let mut out = crate::htslib_mini_rs::hts::hts_open(outfname, c"w".as_ptr());
    let mut header: *mut sam::sam_hdr_t = std::ptr::null_mut();
    ...
    if sam::sam_hdr_write(out, header) < 0 || crate::htslib_mini_rs::hts::hts_close(out) < 0 {
...
pub unsafe fn test_sam_c_1008_test_header_pg_loops() {
    ...
    crate::htslib_mini_rs::hts::hts_set_log_level(crate::htslib_mini_rs::hts::HTS_LOG_OFF);
    for i in 0..2 {
        let in_ = crate::htslib_mini_rs::hts::hts_open(header_texts[i], c"r".as_ptr());
...
pub unsafe fn test_sam_c_1781_test_parse_decimal1(
    ...
    let mut val = crate::htslib_mini_rs::hts::hts_parse_decimal(str_, std::ptr::null_mut(), flags);
    if val != exp {
        libc::fprintf(
            hts_sys::stderr.cast(),
...
pub unsafe fn test_sam_c_1812_test_mempolicy() {
    ...
    fp = crate::htslib_mini_rs::hts::hts_open(fname, c"r".as_ptr());
    bam_fp = crate::htslib_mini_rs::hts::hts_open(bam_name, c"wb".as_ptr());
    ...
    if crate::htslib_mini_rs::hts::hts_close(bam_fp) < 0 {
    ...
    nrecs = 0;
    if crate::htslib_mini_rs::hts::hts_close(fp) < 0 {
        fp = std::ptr::null_mut();
    ...
    bam_fp = crate::htslib_mini_rs::hts::hts_open(bam_name, c"r".as_ptr());
    if crate::htslib_mini_rs::hts::hts_parse_format(&mut cram_fmt, c"cram,no_ref".as_ptr()) < 0 {
    ...
    cram_fp = crate::htslib_mini_rs::hts::hts_open_format(cram_name, c"wc".as_ptr(), &mut cram_fmt);
...
unsafe fn goto_test_mempolicy_cleanup(
    ...
    if !bam_fp.is_null() {
        crate::htslib_mini_rs::hts::hts_close(bam_fp);
    }
    if !cram_fp.is_null() {
        crate::htslib_mini_rs::hts::hts_close(cram_fp);
    }
    for i in 0..nrecs {
        sam::bam_destroy1(recs.add(i));
    }
    ...
    if !(*cram_fmt).specific.is_null() {
        crate::htslib_mini_rs::hts::hts_opt_free((*cram_fmt).specific.cast());
    }
}
...
pub unsafe fn test_sam_c_2031_test_bam_set1_full() {
    ...
    let mut ok = r == 39
        ...
        && (*bam).core.pos == 1000
        && (*bam).core.tid == 1
        && (*bam).core.bin as c_int == crate::htslib_mini_rs::hts::hts_reg2bin(1000, 1010, 14, 5)
        && (*bam).core.qual == 42
