...
use crate::htslib_mini_rs::{faidx, hts::hts_pos_t};
    ...
    for i in 0..nreg {
        ...
        if use_parse_reg != 0 {
            ...
            if use_adjust_reg != 0 {
                ...
                let orig_end = end;
                let r =
                    crate::htslib_mini_rs::faidx::fai_adjust_region(fai, tid, &mut beg, &mut end);
                if r < 0
                    || (((r & 1) != 0) ^ (beg != orig_beg))
                    || (((r & 2) != 0) ^ (end != orig_end))
                {
        ...
        if format == crate::htslib_mini_rs::faidx::FAI_FASTQ {
            ...
            if use_parse_reg != 0 {
                ...
                qual = crate::htslib_mini_rs::faidx::fai_fetchqual64(fai, region, &mut qual_len);
                if qual.is_null() {
                    libc::fprintf(
                        hts_sys::stderr.cast(),
...
pub unsafe fn test_test_faidx_c_366_test_faidx_seq_len64(
    ...
    let found_len = crate::htslib_mini_rs::faidx::faidx_seq_len(fai, seq) as hts_pos_t;
    crate::htslib_mini_rs::faidx::fai_destroy(fai);
    if !expected.is_null() {
        let exp_len = libc::strtoll(expected, std::ptr::null_mut(), 10);
        if found_len != exp_len {
...
pub unsafe fn test_test_faidx_c_430_main(argc: c_int, argv: *mut *mut c_char) -> c_int {
    ...
    loop {
        ...
        match opt as u8 {
            b'i' => fn_ = libc::optarg,
            b'f' => fnfai = libc::optarg,
            b'g' => fngzi = libc::optarg,
            b'o' => fnout = libc::optarg,
            b'e' => expected = libc::optarg,
            b'c' => flags |= faidx::FAI_CREATE,
            b'Q' => format = faidx::FAI_FASTQ,
            b't' => func = libc::optarg,
            b'h' => {
