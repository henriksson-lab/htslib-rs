...
pub unsafe extern "C" fn test_thrash_threads7_c_43_job(v: *mut c_void) -> *mut c_void {
    ...
    crate::htslib_mini_rs::hts::hts_usleep(*usecs as i64);
    std::ptr::null_mut()
}
...
pub unsafe fn test_thrash_threads7_c_49_main(_argc: c_int, _argv: *mut *mut c_char) -> c_int {
    ...
    let p = crate::htslib_mini_rs::thread_pool::hts_tpool_init(num_threads);
    if p.is_null() {
    ...
    for slot in q.iter_mut().take(n_proc) {
        *slot = crate::htslib_mini_rs::thread_pool::hts_tpool_process_init(p, 10, 1);
        if slot.is_null() {
    ...
    loop {
        ...
        count += 1;
        if (count & 15) == 1 {
            libc::fprintf(hts_sys::stderr, c"\r%d ".as_ptr(), count);
            libc::alarm(10);
        }

        for i in 0..num_jobs {
            *t.add(i) = 1000;
            if crate::htslib_mini_rs::thread_pool::hts_tpool_dispatch(
                p,
                q[qnum],
        ...
        crate::htslib_mini_rs::thread_pool::hts_tpool_process_flush(q[qnum]);
        crate::htslib_mini_rs::thread_pool::hts_tpool_process_destroy(q[qnum]);
        libc::free(t.cast());
        q[qnum] = crate::htslib_mini_rs::thread_pool::hts_tpool_process_init(p, 10, 1);
        if q[qnum].is_null() {
            libc::perror(c"hts_tpool_process_init".as_ptr());
            libc::exit(libc::EXIT_FAILURE);
        }
    ...
    for queue in q.iter().take(n_proc) {
        crate::htslib_mini_rs::thread_pool::hts_tpool_process_flush(*queue);
        crate::htslib_mini_rs::thread_pool::hts_tpool_process_destroy(*queue);
    }
    crate::htslib_mini_rs::thread_pool::hts_tpool_destroy(p);
    libc::fprintf(hts_sys::stderr, c"\n".as_ptr());
