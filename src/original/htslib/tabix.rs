...
pub unsafe fn tabix_c_614_main(argc: c_int, argv: *mut *mut c_char) -> c_int {
    ...
    loop {
        ...
        match c {
            x if x == b'R' as c_int => args.regions_fname = libc::optarg,
            x if x == b'T' as c_int => args.targets_fname = libc::optarg,
            x if x == b'C' as c_int => do_csi = 1,
            x if x == b'r' as c_int => reheader = libc::optarg,
            ...
            x if x == b'b' as c_int => {
                conf.bc = libc::strtol(libc::optarg, &mut tmp, 10) as c_int;
                if *tmp != 0 {
                    error_message(format!(
                        "Could not parse argument: -b {}\n",
                        cstr_to_string(libc::optarg)
                    ));
                }
                detect = 0;
            }
            x if x == b'e' as c_int => {
                conf.ec = libc::strtol(libc::optarg, &mut tmp, 10) as c_int;
                if *tmp != 0 {
                    error_message(format!(
            ...
            x if x == b'c' as c_int => {
                conf.meta_char = *libc::optarg as c_int;
                detect = 0;
            }
            x if x == b'f' as c_int => is_force = 1,
            x if x == b'm' as c_int => {
                min_shift = libc::strtol(libc::optarg, &mut tmp, 10) as c_int;
                if *tmp != 0 {
                    error_message(format!(
            ...
            x if x == b'p' as c_int => {
                detect = 0;
                if libc::strcmp(libc::optarg, c"gff".as_ptr()) == 0 {
                    conf = tbx::tbx_conf_gff();
                } else if libc::strcmp(libc::optarg, c"bed".as_ptr()) == 0 {
                    conf = tbx::tbx_conf_bed();
                } else if libc::strcmp(libc::optarg, c"sam".as_ptr()) == 0 {
                    conf = tbx::tbx_conf_sam();
                } else if libc::strcmp(libc::optarg, c"vcf".as_ptr()) == 0 {
                    conf = tbx::tbx_conf_vcf();
                } else if libc::strcmp(libc::optarg, c"gaf".as_ptr()) == 0 {
                    conf = tbx_conf_gaf();
                } else if libc::strcmp(libc::optarg, c"bcf".as_ptr()) == 0 {
                    detect = 1;
                } else if libc::strcmp(libc::optarg, c"bam".as_ptr()) == 0 {
                    detect = 1;
                } else {
                    error_message(format!(
            ...
            x if x == b's' as c_int => {
                conf.sc = libc::strtol(libc::optarg, &mut tmp, 10) as c_int;
                if *tmp != 0 {
                    error_message(format!(
            ...
            x if x == b'S' as c_int => {
                new_line_skip = libc::strtol(libc::optarg, &mut tmp, 10) as c_int;
                if *tmp != 0 {
                    error_message(format!(
                        "Could not parse argument: -S {}\n",
                        cstr_to_string(libc::optarg)
            ...
            3 => {
                let mut v = libc::atoi(libc::optarg);
                if v < 0 {
                    v = 0;
                }
            ...
            4 => {
                args.cache_megs = libc::atoi(libc::optarg);
                if args.cache_megs < 0 {
                    args.cache_megs = 0;
            ...
            x if x == b'@' as c_int => args.threads = libc::atoi(libc::optarg),
    ...
    if list_chroms != 0 {
        return tabix_c_396_query_chroms(*argv.add(libc::optind as usize), args.download_index);
    }

    let fname = *argv.add(libc::optind as usize);
    let ftype = tabix_c_102_file_type(fname);
    if detect != 0 {
        if ftype == IS_GFF {
            conf = tbx::tbx_conf_gff();
        ...
        || !args.targets_fname.is_null()
    {
        ...
        if args.header_only == 0 {
            regs = tabix_c_135_parse_regions(
                args.regions_fname,
                argv.add((libc::optind + 1) as usize),
                argc - libc::optind - 1,
                &mut nregs,
            );
        }
