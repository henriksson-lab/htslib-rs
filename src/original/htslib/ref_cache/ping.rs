...
pub unsafe fn ref_cache_ping_c_39_check_running(port_num: c_int) -> c_int {
    ...
    while !addr.is_null() {
        ...
        loop {
            ...
            if !(s == -1 && *crate::htslib_mini_rs::c_compat::__errno_location() == libc::EINTR) {
                break;
            }
        }
        if s == -1 {
        ...
        loop {
            ...
            if !(res == -1 && *crate::htslib_mini_rs::c_compat::__errno_location() == libc::EINTR) {
                break;
            }
        }
        if res == 0 {
    ...
    if got_connection == 0 {
        if where_ == 2 && *crate::htslib_mini_rs::c_compat::__errno_location() == libc::ECONNREFUSED
        {
            return 0;
        }
