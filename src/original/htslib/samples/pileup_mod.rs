...
use crate::htslib_mini_rs::hts::{htsFile, kstring_t};
...
pub unsafe fn samples_pileup_mod_c_98_main(argc: c_int, argv: *mut *mut c_char) -> c_int {
    ...
    conf.infile = crate::htslib_mini_rs::hts::hts_open(conf.inname, c"r".as_ptr());
    ...
    if conf.in_samhdr.is_null() {
        ...
        crate::htslib_mini_rs::hts::hts_close(conf.infile);
        sam::bam_destroy1(bamdata);
        return ret;
    }
    ...
    loop {
        ...
        for j in 0..depth {
            ...
            if modlen == -1 {
                ...
                crate::htslib_mini_rs::hts::ks_free(&mut insdata);
                return ret;
            }
