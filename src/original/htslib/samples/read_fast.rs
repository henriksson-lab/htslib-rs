...
pub unsafe fn samples_read_fast_c_49_main(argc: c_int, argv: *mut *mut c_char) -> c_int {
    ...
    if argc != 2 {
        samples_read_fast_c_37_print_usage(hts_sys::stdout.cast());
        return ret;
    }
    ...
    if (*infile).format.format != crate::htslib_mini_rs::hts::HTS_FORMAT_FASTA_FORMAT
        && (*infile).format.format != crate::htslib_mini_rs::hts::HTS_FORMAT_FASTQ_FORMAT
    {
    ...
    while c >= 0 {
        ...
        if (*infile).format.format == crate::htslib_mini_rs::hts::HTS_FORMAT_FASTQ_FORMAT {
            libc::printf(c"\nquality: ".as_ptr());
