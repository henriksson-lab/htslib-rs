...
use crate::htslib_mini_rs::hts::hts_pos_t;
...
pub unsafe fn samples_read_fast_index_c_53_main(argc: c_int, argv: *mut *mut c_char) -> c_int {
    ...
    let fmt = if **argv.add(2) == b'Q' as c_char {
        crate::htslib_mini_rs::faidx::FAI_FASTQ
    } else {
    ...
    let idx = crate::htslib_mini_rs::faidx::fai_load3_format(
        inname,
        std::ptr::null(),
        std::ptr::null(),
        crate::htslib_mini_rs::faidx::FAI_CREATE,
        fmt,
    );
    if idx.is_null() {
    ...
    if usemulti == 0 {
        ...
        let mut data = crate::htslib_mini_rs::faidx::fai_fetch64(idx, region, &mut len);
        if data.is_null() {
            if len == -1 {
                libc::printf(c"Failed to get data\n".as_ptr());
                crate::htslib_mini_rs::faidx::fai_destroy(idx);
                return ret;
            }
            ...
            libc::free(data.cast());
            if fmt == crate::htslib_mini_rs::faidx::FAI_FASTQ {
                data = crate::htslib_mini_rs::faidx::fai_fetchqual64(idx, region, &mut len);
                if data.is_null() {
                    if len == -1 {
                        libc::printf(c"Failed to get data\n".as_ptr());
                        crate::htslib_mini_rs::faidx::fai_destroy(idx);
        ...
        loop {
            ...
            let remaining = crate::htslib_mini_rs::faidx::fai_parse_region(
                idx,
                region,
                &mut tid,
                &mut beg,
            ...
            if crate::htslib_mini_rs::faidx::fai_adjust_region(idx, tid, &mut beg, &mut end) == -1 {
                libc::printf(c"Error in adjusting region for tid %d\n".as_ptr(), tid);
                crate::htslib_mini_rs::faidx::fai_destroy(idx);
                return ret;
            }

            let name = crate::htslib_mini_rs::faidx::faidx_iseq(idx, tid);
            let mut len: hts_pos_t = 0;
            let mut data =
                crate::htslib_mini_rs::faidx::faidx_fetch_seq64(idx, name, beg, end, &mut len);
            if data.is_null() {
                if len == -1 {
                    libc::printf(c"Failed to get data\n".as_ptr());
                    crate::htslib_mini_rs::faidx::fai_destroy(idx);
                ...
                libc::free(data.cast());
                if fmt == crate::htslib_mini_rs::faidx::FAI_FASTQ {
                    data = crate::htslib_mini_rs::faidx::faidx_fetch_qual64(
                        idx, name, beg, end, &mut len,
                    );
                    if data.is_null() {
                        if len == -1 {
