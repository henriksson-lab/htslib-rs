...
use crate::htslib_mini_rs::{hts::htsFile, sam};
...
pub unsafe fn samples_write_fast_c_51_main(argc: c_int, argv: *mut *mut c_char) -> c_int {
    ...
    let hfile = crate::htslib_mini_rs::hfile::hopen(outname, mode.as_ptr());
    if hfile.is_null() {
        libc::printf(c"Could not open %s\n".as_ptr(), outname);
        sam::bam_destroy1(bamdata);
        return ret;
    }
    let outfile: *mut htsFile =
        crate::htslib_mini_rs::hts::hts_hopen(hfile, outname, mode.as_ptr());
    if outfile.is_null() {
        libc::printf(c"Could not open %s\n".as_ptr(), outname);
        crate::htslib_mini_rs::hfile::hclose(hfile);
        sam::bam_destroy1(bamdata);
        return ret;
    }
