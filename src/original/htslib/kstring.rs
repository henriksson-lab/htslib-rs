...
// original: ksprintf (htslib/kstring.c:177)
pub unsafe extern "C" fn kstring_c_177_ksprintf(
    s: *mut kstring_t,
    fmt: *const c_char,
    mut ap: ...
) -> c_int {
    let ap = (&mut ap as *mut _).cast::<hts_sys::__va_list_tag>();
    kstring_c_142_kvsprintf(s, fmt, ap)
}
...
pub unsafe fn kstring_c_531_main() -> c_int {
    ...
    kstring_c_177_ksprintf(s, c" abcdefg:    %d ".as_ptr(), 100);
    libc::printf(c"'%s'\n".as_ptr(), (*s).s);
...
mod tests {
    ...
    fn ksprintf_allocates_and_appends_to_kstring() {
        unsafe {
            ...
            assert_eq!(kstring_c_177_ksprintf(&mut s, c"sample-%d".as_ptr(), 42), 9);
            assert!(!s.s.is_null());
            assert_eq!(s.l, 9);
            assert_eq!(CStr::from_ptr(s.s).to_bytes(), b"sample-42");

            assert_eq!(
                kstring_c_177_ksprintf(&mut s, c":%s".as_ptr(), c"ok".as_ptr()),
                3
            );
            assert_eq!(s.l, 12);
