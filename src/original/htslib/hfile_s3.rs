...
pub unsafe fn hfile_s3_c_1112_set_region(ad: *mut s3_auth_data, region: *mut kstring_t) -> c_int {
    let ad = ad.cast::<S3AuthDataLayout>();
    (*ad).region.l = 0;
    (crate::htslib_mini_rs::hts::kputsn((*region).s, (*region).l, &mut (*ad).region) < 0) as c_int
}
...
pub unsafe fn hfile_s3_c_1176_stristr(
    ...
    while *haystack != 0 {
        ...
        while crate::htslib_mini_rs::hts::toupper_c(*h) == crate::htslib_mini_rs::hts::toupper_c(*n)
        {
            h = h.add(1);
            n = n.add(1);
            if *h == 0 || *n == 0 {
