...
use crate::htslib_mini_rs::hts::{kbitset_t, kbs_exists, kputc, kputs, kputsn, kstring_t};
use crate::htslib_mini_rs::vcf::{self, bcf1_t, bcf_hdr_t};
...
pub unsafe fn vcfutils_c_659_bcf_remove_allele_set(
    ...
    let vl_la_lg_lr = (1_u32 << hts_sys::BCF_VL_LA)
        | (1_u32 << hts_sys::BCF_VL_LG)
        | (1_u32 << hts_sys::BCF_VL_LR);
    let vl_a_g_r_la_lg_lr = vl_a_g_r | vl_la_lg_lr;
    let vl_a_r = (1_u32 << hts_sys::BCF_VL_A) | (1_u32 << hts_sys::BCF_VL_R);
    let vl_la_lr = (1_u32 << hts_sys::BCF_VL_LA) | (1_u32 << hts_sys::BCF_VL_LR);
    ...
    *map.add(0) = 0;
    ...
    let mut n_r_new = (*line).n_allele() as c_int - nrm;
    if n_r_new <= 0 {
        hts_sys::hts_log(
    ...
    if i < n_r_ori {
        ...
        if nret > 0 {
            ...
            for _sample in 0..(*line).n_sample() {
                for jj in 0..nret {
    ...
    if num_laa > 0 {
        let num_laa_vals = num_laa / (*line).n_sample();
        laa_map_stride = num_laa_vals + 1;
        let mut max_k = 0;
        laa_map = libc::malloc(
            size_of::<c_int>() * laa_map_stride as usize * (*line).n_sample() as usize,
        )
        .cast::<c_int>();
        if laa_map.is_null() {
            err!();
        }
        lr_orig = libc::malloc(size_of::<c_int>() * (*line).n_sample() as usize).cast::<c_int>();
        if lr_orig.is_null() {
            err!();
        }
        let mut laa_changed = 0;
        for sample in 0..(*line).n_sample() {
        ...
        if laa_changed != 0 {
            ...
            if max_k < num_laa_vals {
                if max_k > 0 {
                    ...
                    new_num_laa = (*line).n_sample() * max_k;
                } else {
                    for sample in 0..(*line).n_sample() {
                        *laa.add(sample as usize) = hts_sys::bcf_int32_missing;
                    }
                    new_num_laa = (*line).n_sample();
                }
            }
            if vcf::bcf_update_format(
                header,
        ...
        if type_ == hts_sys::BCF_HT_STR as c_int {
            ...
            if (vl_a_r_la_lr & (1_u32 << vlen)) != 0 {
                ...
                match vlen {
                    ...
                    x if x == hts_sys::BCF_VL_LA as c_int => {
                        inc = 1;
                        if laa_map.is_null() {
                            err!();
                        }
                    }
                    x if x == hts_sys::BCF_VL_LR as c_int => {
                        if laa_map.is_null() {
                            err!();
                        }
                    }
