...
unsafe extern "C" {
    ...
    #[link_name = "sam_hrecs_find_key"]
    fn htslib_sam_hrecs_find_key(
        type_: *mut c_void,
        key: *const c_char,
        prev: *mut *mut c_void,
    ) -> *mut sam_hrec_tag_layout;
    #[link_name = "stdin"]
...
pub unsafe fn int32_get_blk(b: *mut cram_block, val: *mut i32) -> c_int {
    ...
    if (*block).uncomp_size - (*block).idx < 4 {
        return -1;
    }
    ...
    for i in 0..(*hdr).num_blocks {
        ...
        if !b.is_null()
            && (*(b.cast::<cram_block_layout>())).content_type
                == hts_sys::cram_content_type_EXTERNAL
...
pub unsafe fn cram_cram_io_c_1388_cram_new_block(
    ...
    (*b).method = hts_sys::cram_block_method_RAW;
    (*b).orig_method = hts_sys::cram_block_method_RAW;
...
pub unsafe extern "C" fn cram_cram_io_c_772_uint7_get_32(
    ...
    while nb < 5 && (end.is_null() || p < end as *mut u8) {
        let c = *p;
        p = p.add(1);
        nb += 1;
...
pub unsafe extern "C" fn cram_cram_io_c_788_uint7_get_64(
    ...
    while nb < 10 && (end.is_null() || p < end as *mut u8) {
        let c = *p;
        p = p.add(1);
        nb += 1;
...
pub unsafe fn cram_cram_io_c_1576_cram_uncompress_block(b: *mut hts_sys::cram_block) -> c_int {
    ...
    match (*b).method {
        hts_sys::cram_block_method_RAW => 0,
        hts_sys::cram_block_method_GZIP => {
        ...
        hts_sys::cram_block_method_BZIP2 => -1,
        hts_sys::cram_block_method_LZMA => -1,
        hts_sys::cram_block_method_RANS => {
...
pub unsafe fn cram_cram_io_c_2327_cram_new_metrics() -> *mut hts_sys::cram_metrics {
    ...
    (*m).trial = 2;
    (*m).next_trial = 35;
    (*m).method = hts_sys::cram_block_method_RAW;
    (*m).strat = 0;
    (*m).revised_method = 0;
    (*m).unpackable = 0;
...
pub unsafe fn cram_cram_io_c_2503_bgzf_open_ref(
    ...
    if ((*fp).bitfields & (1 << 30)) != 0 && bgzf_index_load(fp, fn_, c".gzi".as_ptr()) < 0 {
        hts_sys::hts_log(
            ...
            c"bgzf_open_ref".as_ptr(),
            c"Unable to load .gzi index '%s.gzi'".as_ptr(),
            fn_,
        );
        bgzf_close(fp);
        return std::ptr::null_mut();
...
pub unsafe fn cram_cram_io_c_2541_refs_load_fai(
    ...
    if (*r).fp.is_null() {
        hts_sys::hts_log(
            hts_sys::htsLogLevel_HTS_LOG_ERROR,
            c"refs_load_fai".as_ptr(),
            c"Failed to open reference file '%s'".as_ptr(),
            (*r).fn_,
        );
        if r_orig.is_null() {
            cram_cram_io_c_2427_refs_free(r.cast());
    ...
    if fp.is_null() {
        hts_sys::hts_log(
            hts_sys::htsLogLevel_HTS_LOG_ERROR,
            c"refs_load_fai".as_ptr(),
            c"Failed to open index file '%s'".as_ptr(),
            fai_fn.as_ptr(),
        );
        if is_err != 0 {
    ...
    for iref in 0..(*hrecs).nref {
        ...
        if (*r).length != 0 && (*r).length != (*(*hrecs).ref_.add(iref as usize)).len {
            ...
            hts_sys::hts_log(
                ...
                c"sanitise_SQ_lines".as_ptr(),
                c"Header @SQ length mismatch for ref %s, %ld vs %d".as_ptr(),
                (*r).name,
        ...
        if k != (*h).n_buckets {
            *(*r).ref_id.add(iref as usize) = *(*h).vals.add(k as usize);
            ...
            hts_sys::hts_log(
                hts_sys::htsLogLevel_HTS_LOG_WARNING,
                c"refs2id".as_ptr(),
                c"Unable to find ref name '%s'".as_ptr(),
                name,
            );
        }
    }
...
pub unsafe fn cram_cram_io_c_2768_refs_from_header(fd: *mut cram_fd) -> c_int {
    ...
    for i in 0..(*hrecs).nref {
        ...
        *(*r).ref_id.add(j as usize) = e;
        ...
        if !(*h_ref).ty.is_null() {
            let tag = htslib_sam_hrecs_find_key((*h_ref).ty, c"M5".as_ptr(), std::ptr::null_mut());
            if !tag.is_null() {
                (*e).fn_ = cram_string_alloc_c_149_string_dup((*r).pool, (*tag).str_.add(3));
            }

            let tag = htslib_sam_hrecs_find_key((*h_ref).ty, c"LN".as_ptr(), std::ptr::null_mut());
            if !tag.is_null() {
...
pub unsafe fn cram_cram_io_c_1490_cram_block_size(b: *mut hts_sys::cram_block) -> u32 {
    ...
    let header = 2
        ...
        + 4;
    let payload = if (*b).method == hts_sys::cram_block_method_RAW {
        (*b).uncomp_size
    } else {
        (*b).comp_size
    };
...
pub unsafe fn cram_cram_io_c_4330_cram_new_compression_header(
    ...
    (*hdr).td_blk = cram_cram_io_c_1388_cram_new_block(hts_sys::cram_content_type_CORE, 0);
...
pub unsafe fn cram_cram_codecs_c_1377_cram_xpack_decode_expand_char(
    ...
    let b = cram_cram_io_c_1388_cram_new_block(hts_sys::cram_content_type_FILE_HEADER, 0);
    if b.is_null() {
        return -1;
    }
...
pub unsafe fn cram_cram_codecs_c_1537_cram_xpack_encode_store(
    ...
    let tb = cram_cram_io_c_1388_cram_new_block(hts_sys::cram_content_type_FILE_HEADER, 0);
    if tb.is_null() {
        return -1;
    }
...
pub unsafe fn cram_cram_codecs_c_3506_cram_byte_array_len_encode_store(
    ...
    let b_len = cram_cram_io_c_1388_cram_new_block(hts_sys::cram_content_type_FILE_HEADER, 0);
    if b_len.is_null() {
        return -1;
    }
    let store: CramCodecStoreFn =
    ...
    let b_val = cram_cram_io_c_1388_cram_new_block(hts_sys::cram_content_type_FILE_HEADER, 0);
    if b_val.is_null() {
        cram_cram_io_c_1565_cram_free_block(b_len);
        return -1;
    }
    ...
    if hisremote(in_) != 0 {
        if in_l > libc::PATH_MAX as usize {
            hts_sys::hts_log(
                hts_sys::htsLogLevel_HTS_LOG_ERROR,
                c"full_path".as_ptr(),
                c"Reference path is longer than %d".as_ptr(),
                libc::PATH_MAX,
            );
            return;
        }
...
pub unsafe fn cram_cram_external_c_522_cram_block_get_content_id(
    ...
    let b = b.cast::<cram_block_layout>();
    if (*b).content_type == hts_sys::cram_content_type_CORE {
        -1
    } else {
        (*b).content_id
    }
...
mod tests {
    ...
    fn cram_external_slice_header_accessors_read_nullable_outputs_like_c() {
        unsafe {
            let mut hdr = cram_block_slice_hdr_layout {
                content_type: hts_sys::cram_content_type_MAPPED_SLICE,
                ref_seq_id: 17,
                ref_seq_start: 101,
                ref_seq_span: 250,
                num_records: 9,
    ...
    fn cram_external_block_size_and_method_accessors_match_fields() {
        unsafe {
            let mut block = cram_block_layout {
                method: hts_sys::cram_block_method_GZIP,
                orig_method: hts_sys::cram_block_method_RAW,
                content_type: hts_sys::cram_content_type_EXTERNAL,
                content_id: 7,
                comp_size: 12,
                uncomp_size: 18,
                crc32: 99,
            ...
            block.content_type = hts_sys::cram_content_type_CORE;
            assert_eq!(block.content_type, hts_sys::cram_content_type_CORE);
    ...
    fn cram_io_block_lookup_uses_cache_hash_and_linear_collision_fallback() {
        unsafe {
            ...
            let mut ignored_core = cram_block_layout {
                content_type: hts_sys::cram_content_type_CORE,
                content_id: 888,
                ..direct
            };
    ...
    fn cram_io_itf8_ltf8_helpers_match_c_encodings() {
        unsafe {
            ...
            let b = cram_cram_io_c_1388_cram_new_block(hts_sys::cram_content_type_EXTERNAL, 0);
            assert!(!b.is_null());
    ...
    fn cram_io_init_varint_selects_versioned_function_tables() {
        unsafe {
            ...
            assert_eq!(
                vv4.varint_get32.unwrap() as usize,
                cram_cram_io_c_772_uint7_get_32 as usize
            );
            assert_eq!(
                vv4.varint_get32s.unwrap() as usize,
                cram_cram_io_c_780_sint7_get_32 as usize
            );
            assert_eq!(
                vv4.varint_get64.unwrap() as usize,
                cram_cram_io_c_788_uint7_get_64 as usize
            );
            assert_eq!(
                vv4.varint_get64s.unwrap() as usize,
                cram_cram_io_c_796_sint7_get_64 as usize
            );
            assert_eq!(
                vv4.varint_put32.unwrap() as usize,
                cram_cram_io_c_804_uint7_put_32 as usize
            );
            assert_eq!(
                vv4.varint_put32s.unwrap() as usize,
                cram_cram_io_c_808_sint7_put_32 as usize
            );
            assert_eq!(
                vv4.varint_put64.unwrap() as usize,
                cram_cram_io_c_812_uint7_put_64 as usize
            );
            assert_eq!(
                vv4.varint_put64s.unwrap() as usize,
                cram_cram_io_c_816_sint7_put_64 as usize
            );
            assert_eq!(
                vv4.varint_put32_blk.unwrap() as usize,
                cram_cram_io_c_821_uint7_put_blk_32 as usize
            );
            assert_eq!(
                vv4.varint_put32s_blk.unwrap() as usize,
                cram_cram_io_c_831_sint7_put_blk_32 as usize
            );
            assert_eq!(
                vv4.varint_put64_blk.unwrap() as usize,
                cram_cram_io_c_841_uint7_put_blk_64 as usize
            );
            assert_eq!(
                vv4.varint_put64s_blk.unwrap() as usize,
                cram_cram_io_c_851_sint7_put_blk_64 as usize
            );
            assert_eq!(
                vv4.varint_size.unwrap() as usize,
                cram_cram_io_c_768_uint7_size as usize
            );
            assert_eq!(
                vv4.varint_decode32_crc,
                cram_cram_io_c_862_uint7_decode_crc32 as usize as *mut c_void
            );
            assert_eq!(
                vv4.varint_decode32s_crc,
                cram_cram_io_c_907_sint7_decode_crc32 as usize as *mut c_void
            );
            assert_eq!(
                vv4.varint_decode64_crc,
                cram_cram_io_c_953_uint7_decode_crc64 as usize as *mut c_void
            );

            let mut vv3: varint_vec_layout = std::mem::zeroed();
            cram_cram_io_c_5127_cram_init_varint((&mut vv3 as *mut varint_vec_layout).cast(), 3);
            assert_eq!(
                vv3.varint_get32.unwrap() as usize,
                cram_cram_io_c_644_safe_itf8_get as usize
            );
            assert_eq!(
                vv3.varint_get32s.unwrap() as usize,
                cram_cram_io_c_644_safe_itf8_get as usize
            );
            assert_eq!(
                vv3.varint_get64.unwrap() as usize,
                cram_cram_io_c_673_safe_ltf8_get as usize
            );
            assert_eq!(
                vv3.varint_get64s.unwrap() as usize,
                cram_cram_io_c_673_safe_ltf8_get as usize
            );
            assert_eq!(
                vv3.varint_put32.unwrap() as usize,
                cram_cram_io_c_747_safe_itf8_put as usize
            );
            assert_eq!(
                vv3.varint_put32s.unwrap() as usize,
                cram_cram_io_c_747_safe_itf8_put as usize
            );
            assert_eq!(
                vv3.varint_put64.unwrap() as usize,
                cram_cram_io_c_751_safe_ltf8_put as usize
            );
            assert_eq!(
                vv3.varint_put64s.unwrap() as usize,
                cram_cram_io_c_751_safe_ltf8_put as usize
            );
            assert_eq!(
                vv3.varint_put32_blk.unwrap() as usize,
                cram_cram_io_c_620_itf8_put_blk as usize
            );
            assert_eq!(
                vv3.varint_put32s_blk.unwrap() as usize,
                cram_cram_io_c_620_itf8_put_blk as usize
            );
            assert_eq!(
                vv3.varint_put64_blk.unwrap() as usize,
                cram_cram_io_c_632_ltf8_put_blk as usize
            );
            assert_eq!(
                vv3.varint_put64s_blk.unwrap() as usize,
                cram_cram_io_c_632_ltf8_put_blk as usize
            );
            assert_eq!(
                vv3.varint_size.unwrap() as usize,
                cram_cram_io_c_755_itf8_size as usize
            );
            assert_eq!(
                vv3.varint_decode32_crc,
                cram_cram_io_c_196_itf8_decode_crc as usize as *mut c_void
            );
            assert_eq!(
                vv3.varint_decode32s_crc,
                cram_cram_io_c_196_itf8_decode_crc as usize as *mut c_void
            );
            assert_eq!(
                vv3.varint_decode64_crc,
                cram_cram_io_c_501_ltf8_decode_crc as usize as *mut c_void
            );
    ...
    fn cram_io_init_tables_fills_lookup_flags_substitution_and_varints() {
        unsafe {
            ...
            assert_eq!(
                fd4.vv.varint_get32.unwrap() as usize,
                cram_cram_io_c_772_uint7_get_32 as usize
            );
    ...
    fn cram_io_read_write_raw_block_round_trips_v4_layout() {
        unsafe {
            ...
            let b = cram_cram_io_c_1388_cram_new_block(hts_sys::cram_content_type_EXTERNAL, 7);
            assert!(!b.is_null());
            let block = b.cast::<cram_block_layout>();
            (*block).method = hts_sys::cram_block_method_RAW;
            (*block).orig_method = hts_sys::cram_block_method_RAW;
            assert_eq!(
                cram_cram_io_h_248_block_append(b, b"abc".as_ptr().cast(), 3),
                0
            );
            ...
            assert_eq!((*read_block).method, hts_sys::cram_block_method_RAW);
            assert_eq!((*read_block).orig_method, hts_sys::cram_block_method_RAW);
            assert_eq!(
    ...
    fn cram_io_uncompress_block_matches_crc_and_method_rules() {
        unsafe {
            ...
            (*block).method = hts_sys::cram_block_method_GZIP;
            (*block).uncomp_size = 0;
            (*block).crc32_checked = 1;
            assert_eq!(cram_cram_io_c_1576_cram_uncompress_block(b), 0);
            assert_eq!((*block).method, hts_sys::cram_block_method_RAW);

            (*block).method = hts_sys::cram_block_method_BZIP2;
            (*block).uncomp_size = 3;
            assert_eq!(cram_cram_io_c_1576_cram_uncompress_block(b), -1);
            (*block).method = hts_sys::cram_block_method_LZMA;
            assert_eq!(cram_cram_io_c_1576_cram_uncompress_block(b), -1);
            cram_cram_io_c_1565_cram_free_block(b);
    ...
    fn cram_io_new_metrics_initialises_trial_state_like_c() {
        unsafe {
            ...
            assert_eq!((*metrics).method, hts_sys::cram_block_method_RAW);
    ...
    fn cram_codecs_xpack_decode_and_encode_use_maps() {
        unsafe {
            ...
            *out_block.data = 0;
            ...
            let store_block =
                cram_cram_io_c_1388_cram_new_block(hts_sys::cram_content_type_EXTERNAL, 0);
            ...
            let sub_block =
                cram_cram_io_c_1388_cram_new_block(hts_sys::cram_content_type_EXTERNAL, 9);
    ...
    fn cram_io_new_and_free_block_initialise_c_fields() {
        unsafe {
            let b = cram_cram_io_c_1388_cram_new_block(hts_sys::cram_content_type_EXTERNAL, 99);
            assert!(!b.is_null());
            let block = b.cast::<cram_block_layout>();
            assert_eq!((*block).method, hts_sys::cram_block_method_RAW);
            assert_eq!((*block).orig_method, hts_sys::cram_block_method_RAW);
            assert_eq!((*block).content_type, hts_sys::cram_content_type_EXTERNAL);
    ...
    fn cram_codecs_huffman_encode_simple_codes_and_zero_len_noops() {
        unsafe {
            ...
            let out = cram_cram_io_c_1388_cram_new_block(hts_sys::cram_content_type_EXTERNAL, 0);
        ...
        unsafe {
            ...
            assert_eq!(
                CStr::from_ptr(cram_cram_io_c_2378_cram_content_type2str(
                    hts_sys::cram_content_type_CORE
                ))
                .to_bytes(),
                b"CORE"
            );
    ...
    #[test]
    fn cram_io_full_path_preserves_absolute_remote_and_expands_relative_paths() {
        unsafe {
            ...
            out.fill(0);
            let remote = CString::new("http://example.invalid/ref.fa").unwrap();
            cram_cram_io_c_4850_full_path(out.as_mut_ptr(), remote.as_ptr().cast_mut());
            assert_eq!(
                CStr::from_ptr(out.as_ptr()).to_bytes(),
                b"http://example.invalid/ref.fa"
            );
