...
pub unsafe fn samples_split_thread1_c_50_main(args: &[&[u8]]) -> i32 {
    ...
    if args.len() != 3 {
        samples_split_thread1_c_37_print_usage();
        return ret;
    }
    let inname = args[1];
    ...
    // Build NUL-terminated byte buffers for the raw-ptr hts_open API.
    ...
    if bamdata.is_null() {
        ...
        if infile.is_null() {
            ...
            } else if hts::hts_set_threads(infile, 1) < 0
                || hts::hts_set_threads(outfile1, 1) < 0
                || hts::hts_set_threads(outfile2, 2) < 0
            {
