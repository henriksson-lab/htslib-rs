...
pub unsafe fn samples_split2_c_50_main(argc: c_int, argv: *mut *mut c_char) -> c_int {
    let mut mode1 = [b'w' as c_char, 0, 0, 0, 0];
    let mut mode2 = [b'w' as c_char, 0, 0, 0, 0];
    ...
    if argc != 3 {
        samples_split2_c_37_print_usage(crate::htslib_rs::c_compat::stdout.cast());
        return ret;
    }
    let inname = *argv.add(1);
    let outdir = *argv.add(2);

    let size = libc::strlen(outdir) + c"/1.sam.gz".to_bytes_with_nul().len();
    ...
    if file1.is_null() || file2.is_null() {
        ...
        libc::snprintf(file1, size, c"%s/1.sam.gz".as_ptr(), outdir);
        libc::snprintf(file2, size, c"%s/2.sam".as_ptr(), outdir);
        bamdata = sam::bam_init1();
        if bamdata.is_null() {
            libc::printf(c"Failed to initialize bamdata\n".as_ptr());
        } else if sam::sam_open_mode(mode1.as_mut_ptr().add(1), file1, std::ptr::null()) == -1
            || sam::sam_open_mode(mode2.as_mut_ptr().add(1), file2, c"sam.gz".as_ptr()) == -1
        {
            libc::printf(c"Failed to set open mode\n".as_ptr());
        } else {
            infile = hts::hts_open(inname, c"r".as_ptr());
            if infile.is_null() {
                libc::printf(c"Could not open %s\n".as_ptr(), inname);
            } else {
                outfile1 = hts::hts_open(file1, mode1.as_ptr());
                outfile2 = hts::hts_open_format(file2, mode2.as_ptr(), std::ptr::null());
