...
unsafe fn write_bam_from_sam(sam_path: &Path, bam_path: &Path) {
    ...
    let in_fp = hts_open(sam_c.as_ptr(), c"r".as_ptr());
    assert!(!in_fp.is_null(), "failed to open {}", sam_path.display());
    let hdr = sam_hdr_read(in_fp);
    assert!(!hdr.is_null());

    let out_fp = hts_open(bam_c.as_ptr(), c"wb".as_ptr());
...
unsafe fn write_bgz_vcf_from_vcf(vcf_path: &Path, bgz_path: &Path) {
    ...
    let out_fp = hts_open(bgz_c.as_ptr(), c"wz".as_ptr());
...
unsafe fn bam_region_qnames(bam_path: &Path, region: &CStr) -> Vec<String> {
...
unsafe fn bam_linear_scan_qnames_in_region(
    ...
    let target_name = CString::new(contig_name).unwrap();
    for i in 0..(*hdr).n_targets {
        let name = CStr::from_ptr(*(*hdr).target_name.add(i as usize));
...
unsafe fn bcf_csi_region_count(
    ...
    let fp = hts_open(bcf_c.as_ptr(), c"r".as_ptr());
    ...
    let idx = bcf_index_load2(bcf_c.as_ptr(), csi_c.as_ptr());
...
unsafe fn vcf_tabix_region_count(bgz_path: &Path, tbi_path: &Path, region: &CStr) -> usize {
    let bgz_c = c_path(bgz_path);
...
fn round_trip_bam_bai_region_query_matches_linear_scan() {
    unsafe {
        ...
        assert_eq!(sam_index_build(bam_c.as_ptr(), 0), 0);
...
fn round_trip_bcf_csi_region_query_matches_linear_scan() {
    unsafe {
        ...
        let count_indexed = bcf_csi_region_count(&bcf, &csi, c"1", 9_998_999, 10_001_000);
        let count_scan = vcf_linear_scan_count(&vcf, "1", 9_999_000, 10_001_000);
...
fn round_trip_cram_crai_region_query_matches_linear_scan() {
    unsafe {
        ...
        let region = c"CHROMOSOME_II:2976-3070";
...
unsafe fn collect_cram_region_qnames(cram_c: &CStr, ref_c: &CStr, region: &CStr) -> Vec<String> {
    ...
    let idx = sam_index_load(fp, cram_c.as_ptr());
