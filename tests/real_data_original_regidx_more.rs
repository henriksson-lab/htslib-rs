...
fn overlaps(idx: &mut regidx_t, seq: &[u8], beg: i64, end: i64) -> Vec<(i64, i64)> {
    let mut itr = regidx_c_584_regitr_init(idx);

    let ret = regidx_c_401_regidx_overlap(idx, seq, beg, end, Some(&mut itr));
    let mut out = Vec::new();
    while regidx_c_612_regitr_overlap(&mut itr) != 0 {
        assert_eq!(itr.seq.as_slice(), seq);
...
fn tabix_bed_fixture_uses_original_zero_based_half_open_intervals() {
    unsafe {
        ...
        assert_eq!(overlaps(&mut idx, b"X", 1000, 1099), vec![(1000, 1099)]);
        assert_eq!(overlaps(&mut idx, b"X", 1100, 1199), vec![]);
        assert_eq!(
            overlaps(&mut idx, b"X", 1499, 1600),
            vec![(1400, 1499), (1600, 1699)]
        );
        assert_eq!(
            overlaps(&mut idx, b"Y", 100450, 100650),
            vec![
                (100000, 100899),
                (100200, 100699),
                (100400, 100499),
        ...
        assert_eq!(overlaps(&mut idx, b"Z", 100001, 100001), vec![]);
        assert_eq!(
            overlaps(&mut idx, b"Z", 100008, 100008),
            vec![(100008, 100008)]
        );
...
fn tabix_vcf_fixture_auto_parser_indexes_positions_as_points() {
    unsafe {
        ...
        assert_eq!(
            overlaps(&mut idx, b"1", 3_000_149, 3_000_149),
            vec![(3_000_149, 3_000_149)]
        );
        assert_eq!(
            overlaps(&mut idx, b"1", 3_062_914, 3_062_914),
            vec![(3_062_914, 3_062_914), (3_062_914, 3_062_914)]
        );
        assert_eq!(
            overlaps(&mut idx, b"1", 3_106_153, 3_106_153),
            vec![(3_106_153, 3_106_153), (3_106_153, 3_106_153)]
        );
        assert_eq!(
            overlaps(&mut idx, b"1", 3_177_143, 3_177_143),
            vec![(3_177_143, 3_177_143), (3_177_143, 3_177_143)]
        );
        assert_eq!(overlaps(&mut idx, b"1", 3_177_142, 3_177_142), vec![]);
        assert_eq!(
            overlaps(&mut idx, b"2", 3_199_811, 3_199_811),
            vec![(3_199_811, 3_199_811)]
        );
        assert_eq!(
            overlaps(&mut idx, b"4", 3_258_447, 3_258_500),
...
fn tabix_large_chr_vcf_fixture_keeps_high_coordinates_as_zero_based_points() {
    unsafe {
        ...
        assert_eq!(overlaps(&mut idx, b"chr20", 76_960, 76_960), vec![]);
        assert_eq!(
            overlaps(&mut idx, b"chr20", 76_961, 76_961),
            vec![(76_961, 76_961)]
        );
        assert_eq!(
            overlaps(&mut idx, b"chr20", 620_255_099, 630_255_199),
            vec![(620_255_099, 620_255_099), (630_255_199, 630_255_199)]
        );
        assert_eq!(
            overlaps(&mut idx, b"chr20", 2_147_483_646, 2_147_483_646),
            vec![(2_147_483_646, 2_147_483_646)]
        );
        assert_eq!(
            overlaps(&mut idx, b"chr20", 2_147_483_647, 2_147_483_647),
            vec![]
        );
...
fn tabix_bed_fixture_multi_region_queries_match_expected_windows() {
    unsafe {
        ...
        let z = overlaps(&mut idx, b"Z", 99_999, 100_004);
        assert_eq!(
            z,
...
fn annot_tsv_nbp_fixture_matches_exact_regidx_overlaps() {
    unsafe {
        ...
        let dst_rows = overlaps(&mut dst, b"1", 0, 10_000);
        assert_eq!(dst_rows, vec![(999, 3999), (4999, 5999), (6999, 7999)]);

        let src_for_first = overlaps(&mut src, b"1", dst_rows[0].0, dst_rows[0].1);
        ...
        let src_for_second = overlaps(&mut src, b"1", dst_rows[1].0, dst_rows[1].1);
        ...
        let src_for_third = overlaps(&mut src, b"1", dst_rows[2].0, dst_rows[2].1);
...
fn annot_tsv_coordinate_mode_fixture_distinguishes_tab_from_bed_edges() {
    unsafe {
        ...
        assert_eq!(overlaps(&mut tab, b"1", 0, 1), vec![(0, 1)]);
        assert_eq!(overlaps(&mut tab, b"1", 4, 5), vec![(4, 5)]);
        assert_eq!(overlaps(&mut tab, b"1", 6, 6), vec![]);

        assert_eq!(overlaps(&mut bed, b"1", 1, 1), vec![(1, 1)]);
        assert_eq!(overlaps(&mut bed, b"1", 5, 6), vec![(5, 5)]);
        assert_eq!(overlaps(&mut bed, b"1", 0, 0), vec![]);
