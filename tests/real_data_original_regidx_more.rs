use htslib_rs::{
    ...
    regidx_c_91_regidx_seq_nregs, regidx_c_98_regidx_nregs, regidx_parse_f, regidx_t,
};
use std::ffi::CStr;
...
unsafe fn open_index(path: &str, parsef: Option<regidx_parse_f>) -> Box<regidx_t> {
    let fname = fixture_bytes(path);
    let idx = regidx_c_246_regidx_init(Some(&fname), parsef, None, 0, None);
    assert!(
        idx.is_some(),
        "failed to load {}",
...
fn seq_names(idx: &regidx_t) -> Vec<String> {
...
fn overlaps(idx: &mut regidx_t, seq: &CStr, beg: i64, end: i64) -> Vec<(i64, i64)> {
    let mut itr = regidx_c_584_regitr_init(idx);

    let ret = regidx_c_401_regidx_overlap(idx, seq.to_bytes(), beg, end, Some(&mut itr));
    let mut out = Vec::new();
    while regidx_c_612_regitr_overlap(&mut itr) != 0 {
        assert_eq!(itr.seq.as_slice(), seq.to_bytes());
...
fn tabix_bed_fixture_uses_original_zero_based_half_open_intervals() {
    unsafe {
        let mut idx = open_index("htslib/test/tabix/bed_file.bed", None);

        assert_eq!(regidx_c_98_regidx_nregs(&idx), 15);
        assert_eq!(seq_names(&idx), vec!["X", "Y", "Z"]);
        assert_eq!(regidx_c_91_regidx_seq_nregs(&idx, b"X"), 5);
        assert_eq!(regidx_c_91_regidx_seq_nregs(&idx, b"Y"), 5);
        assert_eq!(regidx_c_91_regidx_seq_nregs(&idx, b"Z"), 5);

        assert_eq!(overlaps(&mut idx, c"X", 1000, 1099), vec![(1000, 1099)]);
        assert_eq!(overlaps(&mut idx, c"X", 1100, 1199), vec![]);
        assert_eq!(
            overlaps(&mut idx, c"X", 1499, 1600),
            vec![(1400, 1499), (1600, 1699)]
        );
        assert_eq!(
            overlaps(&mut idx, c"Y", 100450, 100650),
            vec![
                (100000, 100899),
                (100200, 100699),
                (100400, 100499),
        ...
        assert_eq!(overlaps(&mut idx, c"Z", 100001, 100001), vec![]);
        assert_eq!(
            overlaps(&mut idx, c"Z", 100008, 100008),
            vec![(100008, 100008)]
        );
...
fn tabix_vcf_fixture_auto_parser_indexes_positions_as_points() {
    unsafe {
        let mut idx = open_index("htslib/test/tabix/vcf_file.vcf", None);

        assert_eq!(regidx_c_98_regidx_nregs(&idx), 15);
        assert_eq!(seq_names(&idx), vec!["1", "2", "3", "4"]);
        assert_eq!(regidx_c_91_regidx_seq_nregs(&idx, b"1"), 11);
        assert_eq!(regidx_c_91_regidx_seq_nregs(&idx, b"2"), 1);
        assert_eq!(regidx_c_91_regidx_seq_nregs(&idx, b"3"), 1);
        assert_eq!(regidx_c_91_regidx_seq_nregs(&idx, b"4"), 2);

        assert_eq!(
            overlaps(&mut idx, c"1", 3_000_149, 3_000_149),
            vec![(3_000_149, 3_000_149)]
        );
        assert_eq!(
            overlaps(&mut idx, c"1", 3_062_914, 3_062_914),
            vec![(3_062_914, 3_062_914), (3_062_914, 3_062_914)]
        );
        assert_eq!(
            overlaps(&mut idx, c"1", 3_106_153, 3_106_153),
            vec![(3_106_153, 3_106_153), (3_106_153, 3_106_153)]
        );
        assert_eq!(
            overlaps(&mut idx, c"1", 3_177_143, 3_177_143),
            vec![(3_177_143, 3_177_143), (3_177_143, 3_177_143)]
        );
        assert_eq!(overlaps(&mut idx, c"1", 3_177_142, 3_177_142), vec![]);
        assert_eq!(
            overlaps(&mut idx, c"2", 3_199_811, 3_199_811),
            vec![(3_199_811, 3_199_811)]
        );
        assert_eq!(
            overlaps(&mut idx, c"4", 3_258_447, 3_258_500),
...
fn tabix_large_chr_vcf_fixture_keeps_high_coordinates_as_zero_based_points() {
    unsafe {
        let mut idx = open_index("htslib/test/tabix/large_chr.vcf", None);

        assert_eq!(regidx_c_98_regidx_nregs(&idx), 12);
        assert_eq!(seq_names(&idx), vec!["chr11", "chr20"]);
        assert_eq!(regidx_c_91_regidx_seq_nregs(&idx, b"chr11"), 3);
        assert_eq!(regidx_c_91_regidx_seq_nregs(&idx, b"chr20"), 9);
        assert_eq!(overlaps(&mut idx, c"chr20", 76_960, 76_960), vec![]);
        assert_eq!(
            overlaps(&mut idx, c"chr20", 76_961, 76_961),
            vec![(76_961, 76_961)]
        );
        assert_eq!(
            overlaps(&mut idx, c"chr20", 620_255_099, 630_255_199),
            vec![(620_255_099, 620_255_099), (630_255_199, 630_255_199)]
        );
        assert_eq!(
            overlaps(&mut idx, c"chr20", 2_147_483_646, 2_147_483_646),
            vec![(2_147_483_646, 2_147_483_646)]
        );
        assert_eq!(
            overlaps(&mut idx, c"chr20", 2_147_483_647, 2_147_483_647),
            vec![]
        );
...
fn tabix_bed_fixture_multi_region_queries_match_expected_windows() {
    unsafe {
        ...
        let z = overlaps(&mut idx, c"Z", 99_999, 100_004);
        assert_eq!(
            z,
...
fn annot_tsv_nbp_fixture_matches_exact_regidx_overlaps() {
    unsafe {
        let mut src = open_index(
            "htslib/test/annot-tsv/src.9.txt",
            Some(regidx_c_498_regidx_parse_tab),
        );
        let mut dst = open_index(
            "htslib/test/annot-tsv/dst.9.txt",
            Some(regidx_c_498_regidx_parse_tab),
        );

        let dst_rows = overlaps(&mut dst, c"1", 0, 10_000);
        assert_eq!(dst_rows, vec![(999, 3999), (4999, 5999), (6999, 7999)]);

        let src_for_first = overlaps(&mut src, c"1", dst_rows[0].0, dst_rows[0].1);
        ...
        let src_for_second = overlaps(&mut src, c"1", dst_rows[1].0, dst_rows[1].1);
        ...
        let src_for_third = overlaps(&mut src, c"1", dst_rows[2].0, dst_rows[2].1);
...
fn annot_tsv_coordinate_mode_fixture_distinguishes_tab_from_bed_edges() {
    unsafe {
        let mut tab = open_index(
            "htslib/test/annot-tsv/src.14.txt",
            Some(regidx_c_498_regidx_parse_tab),
        );
        let mut bed = open_index(
            "htslib/test/annot-tsv/src.14.txt",
            Some(regidx_c_466_regidx_parse_bed),
        );

        assert_eq!(overlaps(&mut tab, c"1", 0, 1), vec![(0, 1)]);
        assert_eq!(overlaps(&mut tab, c"1", 4, 5), vec![(4, 5)]);
        assert_eq!(overlaps(&mut tab, c"1", 6, 6), vec![]);

        assert_eq!(overlaps(&mut bed, c"1", 1, 1), vec![(1, 1)]);
        assert_eq!(overlaps(&mut bed, c"1", 5, 6), vec![(5, 5)]);
        assert_eq!(overlaps(&mut bed, c"1", 0, 0), vec![]);
